...
import { type User, createJobListing } from '../../../services/firebase';
import { type UserProfile, type JobListing } from '../../../types';
import Button from '../../Button';
import CloseIcon from '../../icons/CloseIcon';

interface CreateJobListingModalProps {
  onClose: () => void;
  user: User;
...
const CreateJobListingModal: React.FC<CreateJobListingModalProps> = ({ onClose, user, userProfile, onListingCreated }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const newListingData: Omit<JobListing, 'id' | 'createdAt' | 'status' | 'listingType' | 'applicants' | 'applicantCount'> = {
                recommenderId: user.uid,
                jobTitle,
                contactPerson,
                contactPhone,
                contactEmail,
                recommenderName: userProfile.fullName || userProfile.email.split('@')[0],
                // Fix: Initialize the 'ratedBy' property as an empty array to satisfy the type definition.
                ratedBy: [],
                ...(areaCode && { areaCode }),
                ...(jobDetails && { jobDetails }),
                ...(companyName && { companyName }),
                ...(address && { address }),
                ...(userProfile.avatarUrl && { recommenderAvatarUrl: userProfile.avatarUrl }),
                ...(salaryData && { salary: salaryData }),
            };
            
            await createJobListing({ ...newListingData, applicants: [], applicantCount: 0 });
            
            onListingCreated();
            onClose();

        } catch (err) {
            setError('No se pudo crear la recomendación. Inténtalo de nuevo.');
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
            <div
                ...
                role="dialog"
                aria-modal="true"
                aria-labelledby="create-job-listing-title"
            >
                <div className="relative p-6 sm:p-8">
                    ...
                    <h2 id="create-job-listing-title" className="text-2xl font-bold text-gray-900 dark:text-white mb-4">Recomendar un Empleo</h2>
                     ...
                     <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                        ...
                        <div className="flex justify-end pt-4 gap-3">
                           ...
                           <Button type="submit" disabled={loading}>
                             {loading ? 'Publicando...' : 'Crear Recomendación'}
                           </Button>
                        </div>
                    </form>
                </div>
